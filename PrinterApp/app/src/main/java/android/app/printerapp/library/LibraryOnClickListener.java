...
//        arg2--;
                                ...
                                .callback(new MaterialDialog.ButtonCallback() {
                                    ...
                                    public void onPositive(MaterialDialog dialog) {
                                        ...
                                        if (p != null) {
                                            ...
                                            //lkj Toast.makeText(mContext.getActivity(), "Loading " + f.getName() + " in " + p.getDisplayName(), Toast.LENGTH_LONG).show();
                                            Toast.makeText(mContext.getActivity(), "Loading " + f.getName() + " in " + "BBP", Toast.LENGTH_LONG).show();
                                        } else {
    ...
    private void showOptionPopUp(View view, final int index) {
        ...
        final File gcode_file = new File(f.getAbsolutePath() + "/_gcode/" + f.getName() + ".gco"); //lkj
        final File gcode_file2 = new File(f.getAbsolutePath() + "/_gcode/" + f.getName() + ".gcode"); //lkj

        Log.d("lkj showOptionPopUp", "file name:" + f.getName());
        Log.d("lkj showOptionPopUp", "file getAbsolutePath:" + f.getAbsolutePath());

        //Different pop ups for different type of files
        if (gcode_file.exists() || gcode_file2.exists() || f.getParent().equals("sd") || f.getParent().equals("local")) { //lkj
            popup.getMenuInflater().inflate(R.menu.library_model_menu_local, popup.getMenu());
            Log.d("lkj showOptionPopUp", "display send to printer menu ");
        } else {
            ...
            if (f.isDirectory()){
                ...
                if (!LibraryController.isProject(f)){
                    ...
                    if (mContext.getCurrentTab().equals(LibraryController.TAB_FAVORITES)){
                      //lkj  popup.getMenu().findItem(R.id.library_model_delete).setVisible(false);

...
public class SlicingHandler {
    ...
    private String mCurFilePath = null; //lkj
    ...
    public void setOriginalProject2222(String path) {

        mOriginalProject = path;
        Log.i("OUT", "Workspace: " + path);
    }

    public void setOriginalProject(String projectPath, String fileAbsPath) { //lkj

        mOriginalProject = projectPath;
        mCurFilePath = fileAbsPath;
        Log.i("lkj setOriginalProject", "Workspace: " + projectPath + ", mCurFilePath:" + mCurFilePath);
    }
...
//                            new SaveTask().execute();
    ...
    private Runnable mSaveRunnable = new Runnable() {
        ...
        public void run() {
          ...
          //lkj  File mFile = createTempFile();
            File file = new File(mCurFilePath);
            setLastReference(file.getAbsolutePath());
            DatabaseController.handlePreference(DatabaseController.TAG_SLICING, "Last", file.getName(), true);

            Log.i("Slicer", "Sending slice command");
            OctoprintSlicing.sliceCommand(mActivity,mPrinter.getAddress(),file,mExtras);

...
public class DevicesListController {
    ...
    public static ModelPrinter mPrinter;
    ...
    public static void addToList(ModelPrinter m) {
        ...
        Log.i("lkj-addToList", "Added " + m.getId());
    ...
    public static ModelPrinter getPrinter(long id) {
       ...
       /* for (ModelPrinter p : mList) {
            ...
            if (DatabaseController.checkExisting(p)) {
                ...
                if (p.getId() == id) {
                        Log.d("lkj getPrinter", "return a good printer");
                    return p;
                }
            }

        }
        Log.d("lkj getPrinter", "return a null printer!!!!!!");
        return null;
        */
    }
    ...
    public static void loadList(final Context context) {
        ...
        while (!c.isAfterLast()) {
              ...
              //lkj
            Log.i("lkj-loadList", "Entry: id" + c.getInt(0) + c.getString(1) + ";" + c.getString(2) + ";" + c.getString(3));

            final ModelPrinter m;
            if(c.getString(1).contains("Fastbot")){
                Log.i("lkj-loadList", "get fastbot");
                m = mPrinter;
            } else {
                m = new ModelPrinter(c.getString(1), c.getString(2), Integer.parseInt(c.getString(5)));
            }

            if (Integer.parseInt(c.getString(5)) == StateUtils.TYPE_CUSTOM) {

               //lkj  m.setTypeAndProfile(StateUtils.TYPE_CUSTOM, c.getString(6));
            }
    ...
    public static void selectPrinter(Context c, File f, final SlicingHandler slicer) {
        ...
        if (printersList.length > 1){
            ...
            MainActivity.performClick(2);
         //lkj   MainActivity.showExtraFragment(1, m.getId());
    ...
    public static ModelPrinter selectAvailablePrinter(int type, String typeName) {
        ...
        for (ModelPrinter p : DevicesListController.getList()) {
               if (p.getProfile().equals(typeName)) {
                   if (p.getStatus() == StateUtils.STATE_OPERATIONAL) {
                       Log.d("lkj selectAvailablePrinter", "get a available printer");
                       return p;
                   }
               }
        }

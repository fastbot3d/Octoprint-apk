...
public class MainActivity extends ActionBarActivity {
    ...
    private PrintViewFragment mPrintViewFragmentFragment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // HandlerThread updateUIHanderThread = new HandlerThread("updateUI");
       // updateUIHanderThread.start();
        //mUpdateUIHandler = new UpdateUIHandler(updateUIHanderThread.getLooper());
        mUpdateUIHandler = new UpdateUIHandler(this.getMainLooper());
        DevicesListController.mPrinter.mUpdateUIHandler =this.mUpdateUIHandler;
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(mAdapterNotification, new IntentFilter("notify"));
        ...
        //MIN_PRIORITY(1)  NORM_PRIORITY(5) MAX_PRIORITY(10) lkj

    }

    /* lkj
        ModelPrinter p = new ModelPrinter("Fastbot", "127.0.0.1", StateUtils.STATE_NEW);
        String api = DatabaseController.getPreference(DatabaseController.TAG_KEYS,PrintNetworkManager.getNetworkId(p.getName()));
        if ( api == null ){
            DatabaseController.handlePreference(DatabaseController.TAG_KEYS, PrintNetworkManager.getNetworkId(p.getName()), "C037A5E724064E83B22920BC41C93EF2", true);
            if (!DatabaseController.checkExisting(p)) {
                DatabaseController.writeDb(p.getName(), p.getAddress(), String.valueOf(p.getPosition()), String.valueOf(p.getType()),
                        "ssid-fastbot");
            }
        }
        String a = DatabaseController.getPreference(DatabaseController.TAG_KEYS, PrintNetworkManager.getNetworkId(p.getName()));
    ...
    public void onItemSelected(int id) {
        ...
        Log.d("lkj-onItemSelected", "Pressed " + id);
        //start transaction
        ...
        switch (id) {
            ...
            case 2: {
                ...
                //}
                if (getFragmentManager().findFragmentByTag(ListContent.ID_PRINTVIEW) == null) {
                    mPrintViewFragmentFragment = new PrintViewFragment();
                    fragmentTransaction.add(R.id.maintab3, mPrintViewFragmentFragment, ListContent.ID_PRINTVIEW);
                }
                mCurrent = mPrintViewFragmentFragment;
        ...
        if (mCurrent != null) {
            Log.d("lkj-onItemSelected", "Changing " + mCurrent.getTag());
    ...
    public static void showExtraFragment(int type, long id) {
        ...
        switch (type) {
            ...
            case 1:
                ...
                PrintViewFragment detailp = null;
                if (fragment == null) {
                    detailp = new PrintViewFragment();
                } else {
                    detailp = (PrintViewFragment) fragment;
                }
    ...
    {
        Log.i("lkj-updateUI", "message= " + message);

        if (message!=null) {
            if (message.equals("Devices")){
           //lkj     if (mDevicesFragment!=null) mDevicesFragment.notifyAdapter();

                //Refresh printview fragment if exists
                if ((mPrintViewFragmentFragment !=null) && (mCurrent == mPrintViewFragmentFragment) ) {
                  //  Fragment fragment = mManager.findFragmentByTag(ListContent.ID_PRINTVIEW);
                   // if (fragment != null) ((PrintViewFragment) fragment).refreshData();
                    mPrintViewFragmentFragment.refreshData();
                }
                //if (fragment != null) ((PrintViewFragment) fragment).handler.sendEmptyMessage(1);
    ...
    private BroadcastReceiver mAdapterNotification = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i("lkj-BroadcastReceiver", "message= " + message);
 ...
 */
    ...
    protected void onDestroy() {
        ...
        this.unregisterReceiver(mLocaleChange);
